...
# presto installer
# Usage: curl -fsSL https://raw.githubusercontent.com/candril/presto/main/scripts/install.sh | bash
#
#   PRESTO_VERSION=0.2.0              install a specific release instead of the latest
#   PRESTO_INSTALL_DIR=~/.local/bin   install somewhere other than /usr/local/bin
#
...
REPO="candril/presto"
INSTALL_DIR="${PRESTO_INSTALL_DIR:-/usr/local/bin}"
BINARY_NAME="presto"
...
ASSET="presto-${OS}-${ARCH}.gz"

echo "presto installer"
echo "  OS:      ${OS}"
echo "  Arch:    ${ARCH}"
echo "  Install: ${INSTALL_DIR}/${BINARY_NAME}"
echo

if [ -n "${PRESTO_VERSION:-}" ]; then
  TAG="v${PRESTO_VERSION#v}"
  echo "Installing ${TAG}..."
else
...
if [ -w "$INSTALL_DIR" ]; then
  mv "presto-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
else
  echo "  ${INSTALL_DIR} is not writable — using sudo"
  sudo mv "presto-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
fi

echo
echo "presto ${TAG} installed to ${INSTALL_DIR}/${BINARY_NAME}"
if ! command -v presto >/dev/null 2>&1; then
  echo "Note: ${INSTALL_DIR} is not on your PATH."
fi
echo "Run 'presto --help' to get started."
