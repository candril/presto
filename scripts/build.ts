...
if (olderThan(Bun.version, BUN_FLOOR)) {
  console.error(`presto needs bun ${BUN_FLOOR} or newer to build; this is ${Bun.version}`)
  process.exit(1)
}
...
// `dist/presto`; CI and `--all` builds carry the platform in the name. (Copying the
...
for (const target of targets) {
  const outfile = localBuild ? "dist/presto" : `dist/presto-${target.os}-${target.arch}`
  console.log(`presto ${version} → ${outfile}`)
  const result = await Bun.build({
    ...
    define: {
      ...
      PRESTO_VERSION: JSON.stringify(version),
    },
  })
  if (!result.success) {
