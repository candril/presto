...
underline, strikethrough), which is everything presto draws with. Pillow renders the
cells in Menlo on the app's own background, so the result is the screenshot a
...
def captioned(img, keycap, text):
    ...
    they did — without it the demo is a list flickering through states nobody can
    name. Over the list rather than in a strip beneath it, so the caption reads at a
...
FOCUS_BG = (36, 40, 59)  # theme.headerBg — the selected row, but also the header and status bars
MARK = (224, 175, 104)  # theme.warning, off-hue from anything presto draws itself


def focus_box(lines):
    """Pixel bounds of the selected row, or None. The header, tab and status bars wear
    the same background across the full width; a PR row is told apart by the "(#123)"
    suffix every title carries."""
    for r, row in enumerate(lines):
        text = "".join(ch for ch, _ in row)
        if "(#" not in text:
            continue
        cols = [c for c, (_, st) in enumerate(row) if st.bg == FOCUS_BG]
        if not cols:
            continue
        return (PAD + min(cols) * CW, PAD + r * LH, PAD + (max(cols) + 1) * CW, PAD + (r + 1) * LH)
    return None


def mark_move(img, box, prev):
    """Ring the row under the cursor and, when it has just jumped, point at where it
    came from. A jump down the list is otherwise a cut between two dense stills:
    nothing tells the eye which of twenty rows is the one that changed."""
    if box is None:
        return img
...
def gif(frames, out, cols, rows):
    ...
    for path, seconds, keycap, caption, mark in frames:
        ...
        rendered.append(img.resize((img.width // 2, img.height // 2), Image.LANCZOS))
        durations.append(int((hold_for(caption) if seconds == "auto" else float(seconds)) * 1000))
    # One palette for every frame, built from a strip of all of them: with a shared
    # palette the writer stores only the cells that changed between frames, which is
    # most of what keeps a twenty-frame tour under a megabyte.
    strip = Image.new("RGB", (rendered[0].width, rendered[0].height * len(rendered)))
    for i, img in enumerate(rendered):
        strip.paste(img, (0, i * img.height))
    palette = strip.quantize(colors=128, method=Image.Quantize.MEDIANCUT, dither=Image.Dither.NONE)
    images = [img.quantize(palette=palette, dither=Image.Dither.NONE) for img in rendered]
    images[0].save(
        out,
        save_all=True,
